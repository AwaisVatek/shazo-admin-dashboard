...
interface TopupRequest {
  ...
  riderName: string;
  amount: number;
  paymentChannel: 'easypaisa' | 'jazzcash' | 'bank_transfer';
  referenceNo: string;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-brand-navy-800 p-5 rounded-xl border border-slate-800">
          ...
          <span className="text-2xl font-mono font-black text-white mt-1 block">92,100 PKR</span>
        </div>
        <div className="bg-brand-navy-800 p-5 rounded-xl border border-slate-800">
          <span className="text-xs text-slate-500 uppercase font-bold block">Outstanding Fleet Wallets</span>
          <span className="text-2xl font-mono font-black text-emerald-400 mt-1 block">841,500 PKR</span>
        </div>
        <div className="bg-brand-navy-800 p-5 rounded-xl border border-slate-800">
          ...
          <span className="text-2xl font-mono font-black text-yellow-500 mt-1 block">
            {topups.filter(t=>t.status==='pending').length} Requests
          </span>
        </div>
      </div>
            ...
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="bg-slate-905 border-b border-slate-800 text-xs font-semibold uppercase tracking-wider text-slate-400">
                <tr>
                  ...
                  <th className="px-6 py-4">Rider Brand</th>
                ...
                {topups.length === 0 ? (
                  ...
                  <tr key={t.id} className="hover:bg-slate-900/40 transition">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-white block">{t.riderName}</span>
                      <span className="text-[10px] text-slate-500 block uppercase font-bold mt-0.5">{t.paymentChannel}</span>
                    </td>
        ...
        <div className="space-y-6">
          <div className="bg-brand-navy-800 rounded-xl border border-slate-800 p-6 space-y-4">
            ...
            <form onSubmit={handleManualAdjustment} className="space-y-4 text-sm">
              <div>
                <label className="text-xs text-slate-400 font-bold uppercase">Rider Identity (e.g., RIDER-101)</label>
                <input
                  type="text"
                  required
