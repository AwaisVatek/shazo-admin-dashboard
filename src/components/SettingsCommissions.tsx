...
export const SettingsCommissions: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const loadedCommissions = data && data.length > 0 ? data : [];
      
      const completeCommissions = ALL_SERVICES.map(svc => {
        const existing = loadedCommissions.find((c: any) => c.serviceType === svc.key || c.service_type === svc.key);
        return {
          serviceType: svc.key,
          serviceLabel: svc.label,
          commissionPercentage: Number(existing?.commissionPercentage || existing?.commission_rate || 0),
          minCommissionPKR: Number(existing?.minCommissionPKR || existing?.minimum_platform_cut || 0),
          variablePeakHourlyCharge: existing?.variablePeakHourlyCharge || existing?.commission_type === 'variable' || false,
        };
      });
      setCommissions(completeCommissions);
    } catch (err: any) {
      ...
      setCommissions(ALL_SERVICES.map(svc => ({
          serviceType: svc.key,
          serviceLabel: svc.label,
          commissionPercentage: 0,
          minCommissionPKR: 0,
          variablePeakHourlyCharge: false,
      })) as any);
    } finally {
      setLoading(false);
    }
            ...
            <div key={comm.serviceType} className="bg-brand-navy-800 p-5 rounded-xl border border-slate-850 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center space-x-3 w-56 flex-shrink-0">
                <div className="p-2 bg-slate-905 rounded-lg border border-slate-800/60">
                  {comm.serviceType.includes('bike') && <Bike className="w-5 h-5 text-indigo-400" />}
                  {comm.serviceType.includes('car') && <Car className="w-5 h-5 text-emerald-400" />}
                  {comm.serviceType.includes('rickshaw') && <Car className="w-5 h-5 text-amber-500" />}
                  {comm.serviceType === 'ambulance' && <Shield className="w-5 h-5 text-rose-500" />}
                  {comm.serviceType === 'food_delivery' && <Flame className="w-5 h-5 text-amber-500" />}
                </div>
                <div>
                  <span className="text-white font-black tracking-wide text-sm">{comm.serviceLabel} Split</span>
